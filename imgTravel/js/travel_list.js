...
$(function(){
	...
	$("#travel_focus .preNext").css("opacity",0.2).hover(function() {
  	...
  	function toPre(e){//判断鼠标是否在图片的左半部分
  		if(e.screenX < 677 && e.screenX > 196 && e.screenY <　(937-$(window).scrollTop()) && e.screenY > 350-$(window).scrollTop()){
  			return true;
  		}
  		return false;
  	}
  	function toNext(e){//判断鼠标是否在图片的左半部分
  		if(e.screenX < 1146 && e.screenX > 677 && e.screenY < 937-$(window).scrollTop() && e.screenY > 350-$(window).scrollTop()){
  			return true;
  		}
  		return false;
  	}
  	...
  	$(document).click(function(e){
  		...
  		if(toPre(e)){
  			$("#travel_focus .pre").click();
  		}
  		else if(toNext(e)){
  			$("#travel_focus .next").click();
  		}
  	})
	...
	function showPics(index) { //普通切换
		...
		$("#travel_focus ul").stop(true,false).animate({"left":nowLeft},300).eq(index).fadeIn(1000); //通过animate()调整ul元素滚动到计算出的position
